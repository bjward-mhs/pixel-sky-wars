...
const ARTILLERY_VIEW_WIDTH = 440;
...
function generateWorld() {
  const groundLine = 112;
  terrain = Array.from({ length: WORLD_HEIGHT }, (_, y) => Array.from({ length: WORLD_WIDTH }, () => {
    if (y < groundLine) return y > 94 && random() > .84 ? 'mist' : 'sky';
    const depth = y - groundLine;
    return depth < 5 ? (random() > .7 ? 'grass' : 'mud') : depth % 9 === 0 ? 'wetMud' : depth > 40 ? 'darkMud' : 'mud';
  }));
  trenches = [];
  [48, 180, 312, 444, 576].forEach((start, index) => {
    ...
    for (let x = start; x < start + width; x += 1) {
      ...
      for (let y = groundLine; y < Math.min(WORLD_HEIGHT, trenchFloor + 1); y += 1) terrain[y][x] = y > trenchFloor - 3 ? 'darkMud' : 'mud';
    }
  });
  ...
  player.x = 48; player.y = 80; player.velocityY = 0; player.health = 100; player.skin = skinTones[Math.floor(random() * skinTones.length)]; player.direction = 1; player.cooldown = 0; player.artilleryCooldown = 0; player.alive = true;
  for (let index = 0; index < 6; index += 1) units.push(makeUnit(70 + index * 13, 'allied'));
  for (let index = 0; index < 9; index += 1) units.push(makeUnit(610 + index * 8, 'enemy'));
  seedReadout.textContent = `SEED ${String(Math.abs(Math.floor(seed))).padStart(6, '0').slice(-6)}`;
  cameraX = 0; gameOver = false; artilleryMode = false; objective.textContent = 'Advance through the trenches. Keep the original line.'; updateArtilleryReadout();
  versionReadout.textContent = `v${GAME_VERSION}`;
}
...
function groundHeightAt(x) { const column = terrain[Math.max(0, Math.min(WORLD_WIDTH - 1, Math.floor(x)))]; for (let y = 75; y < WORLD_HEIGHT; y += 1) if (column?.[y] !== 'sky' && column?.[y] !== 'cloud' && column?.[y] !== 'mist') return y; return 112; }
function viewWidth() { return artilleryMode ? ARTILLERY_VIEW_WIDTH : NORMAL_VIEW_WIDTH; }
...
function updateArtilleryReadout() { artilleryReadout.textContent = artilleryMode ? 'ARTILLERY VIEW / LEFT CLICK TO FIRE / E TO EXIT' : 'RIFLE READY / FIND THE GUN'; }
...
function updateTrenches(delta) { trenches.forEach(trench => { const occupants = units.concat(player).filter(unit => unit.alive && unit.x > trench.start + 3 && unit.x < trench.start + trench.width - 3); const allied = occupants.filter(unit => unit.team === 'allied').length; const enemy = occupants.filter(unit => unit.team === 'enemy').length; if (allied && !enemy && trench.owner !== 'allied') trench.capture = Math.min(100, trench.capture + delta * 12); if (enemy && !allied && trench.owner !== 'enemy') trench.capture = Math.max(-100, trench.capture - delta * 12); if (trench.capture >= 100) trench.owner = 'allied'; if (trench.capture <= -100) trench.owner = 'enemy'; }); const original = trenches[0]; frontReadout.textContent = `FRONT LINE / ${String(trenches.filter(trench => trench.owner === 'allied').length).padStart(2, '0')}`; if (original.owner === 'enemy' && !gameOver) { gameOver = true; objective.textContent = 'The original trench was captured.'; sound('blast'); } }
...
regenerateButton.addEventListener('click', () => { seed = Math.floor(Math.random() * 1000000); generateWorld(); startAudio(); });
